<template>
  <v-card>
        ...
        <v-layout row wrap>
          <v-flex xs12>
            <v-radio-group v-model="color" row>
              <v-radio v-for="(colorValue, i) in ['success', 'info', 'error', 'cyan darken-2']" :key="i"
                :color="colorValue" :label="colorValue" :value="colorValue">
              </v-radio>
            </v-radio-group>
          </v-flex>
    ...
    <v-snackbar v-model="snackbar" :color="color" :multi-line="mode === 'multi-line'"
      :timeout="timeout" :vertical="mode === 'vertical'"> {{ text }}
