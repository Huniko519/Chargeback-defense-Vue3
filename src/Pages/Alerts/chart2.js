...
export default {
    ...
    methods: {
        loadData(){
            let thedata = this.getCustomerAlerts
    ...
    mounted() {
        ...
        //this.renderChart(this.datacollection, this.options)
        let rb = this.$store.state.warpliteapi.customerAlertsLoaded
        if (rb) {
            this.loadData()
        } else {
            let unwatch = this.$store.watch((state) => state.warpliteapi.customerAlertsLoaded , () => {
                this.loadData()
                unwatch()
            })
        }
